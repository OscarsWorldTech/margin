...
type Book={id:string;title:string;author:string;duration:number;cover?:string;status?:string;tracks:{index:number;startOffset:number;duration:number;url:string}[];chapters:{id:number;title:string;start:number;end:number}[];position:number;cues:Cue[];readalong:Readalong|null;notes:Note[];job:Job};
...
export default function Home(){
 ...
 <div className="reader-heading"><Button variant="ghost" onClick={()=>setView('library')}><ArrowLeft/> Library</Button><div><h1>{book.title}</h1><span>{book.author}</span></div><span className="reader-tag">{readingBook?'Imported book text':status.demo?'Prepared sample captions':'Audio captions'}</span></div>
