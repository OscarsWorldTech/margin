...
import {StorytellerImport, type Readalong} from '@/components/storyteller-import';
import {EbookAlignment, type Alignment} from '@/components/ebook-alignment';
...
type Book={id:string;title:string;author:string;duration:number;cover?:string;status?:string;tracks:{index:number;startOffset:number;duration:number;url:string}[];chapters:{id:number;title:string;start:number;end:number}[];position:number;cues:Cue[];readalong:Readalong|null;alignment:Alignment;notes:Note[];job:Job};
...
export default function Home(){
 ...
 const [textSource,setTextSource]=useState<'book'|'captions'>('book');
 const readingBook=textSource==='book'&&Boolean(book?.readalong);
 const readingCues=readingBook?book!.readalong!.cues:book?.cues||[];
 const refreshAlignment=useCallback(async()=>{if(!book)return;const fresh=await api<Book>('/books/'+book.id);setBook(b=>b?.id===fresh.id?{...b,cues:fresh.cues,job:fresh.job,alignment:fresh.alignment}:b);},[book?.id]);
 const active=activeCue(readingCues,position-offset);
 function changeTextSource(value:'book'|'captions'){setTextSource(value);setSelected([]);setDraft('');setReadPage(0);setCaptionQuery('');setOffset(0);}
 const chosen=selected.slice().sort((a,b)=>a-b).map(i=>readingCues[i]).filter(Boolean) as Cue[];
 const quote=chosen.map(c=>c.text).join(' ');
 const filteredCues=readingCues.map((c,i)=>({c,i})).filter(({c})=>c.text.toLowerCase().includes(captionQuery.toLowerCase()));
 ...
 async function openBook(id:string){
   ...
   await act(async()=>{
     ...
     changeTextSource('book');setBook(b);setPosition(b.position);setTrack(trackAtPosition(b.tracks,b.position));
 ...
 function replay(cue:Cue){playIntent.current=true;seek(cue.start+offset);if(native){void player.current?.play().catch(()=>{playIntent.current=false;setError('Audio could not play.');});return;}if(seekTo.current===null)void audio.current?.play().catch(()=>{playIntent.current=false;setError('Audio could not play.');});}
 ...
 <div className="reader-heading"><Button variant="ghost" onClick={()=>setView('library')}><ArrowLeft/> Library</Button><div><h1>{book.title}</h1><span>{book.author}</span></div><span className="reader-tag">{readingBook?'Book text':status.demo?'Prepared sample captions':'Audio captions'}</span></div>
 <div className="reading-layout"><section className="transcript-panel"><Tabs defaultValue="transcript"><div className="reading-toolbar"><TabsList variant="line"><TabsTrigger value="transcript">Read along</TabsTrigger><TabsTrigger value="chapters">Chapters</TabsTrigger></TabsList><Button variant={follow?'secondary':'ghost'} onClick={()=>{if(!follow)setCaptionQuery('');setFollow(!follow);}}>{follow&&<Check/>} Follow audio</Button></div><TabsContent value="transcript"><div className="caption-tools"><label className="search-box"><Search/><Input aria-label="Search passages" placeholder="Find a passage…" value={captionQuery} onChange={e=>{setCaptionQuery(e.target.value);setReadPage(0);setFollow(false);}}/></label><Button variant="outline" disabled={busy||status.demo||Boolean(running)} onClick={transcribe}>{book.job?.status==='paused'||book.job?.status==='failed'?'Resume captions':book.cues.length?'Captions ready':'Generate captions'}</Button><label className="file-button">Import SRT / VTT<input type="file" accept=".srt,.vtt" disabled={status.demo||Boolean(running)} onChange={e=>{const f=e.target.files?.[0];if(f)void act(async()=>{const r=await api('/books/'+book.id+'/captions',{text:await f.text()});setBook(b=>b?.id===book.id?{...b,cues:r.cues,job:null}:b);changeTextSource('captions');});e.target.value='';}}/></label><StorytellerImport key={book.id} title={book.title} duration={book.duration} tracks={book.tracks} chapters={book.chapters} disabled={busy||status.demo} existing={Boolean(book.readalong)} onSeek={t=>replay({start:t-offset,end:t-offset+1,text:''})} onSave={async value=>{const readalong=await api<Readalong>('/books/'+book.id+'/readalong',value);setBook(b=>b?.id===book.id?{...b,readalong}:b);setBooks(bs=>bs.map(b=>b.id===book.id?{...b,status:'readalong'}:b));changeTextSource('book');setNotice('Book text imported. Your captions and notes are kept.');}}/>
 <EbookAlignment key={'ebook-'+book.id} bookId={book.id} title={book.title} demo={status.demo} initial={book.alignment} api={api} onRefresh={refreshAlignment} onSeek={t=>replay({start:t-offset,end:t-offset+1,text:''})} onActivated={readalong=>{setBook(b=>b?.id===book.id?{...b,readalong,alignment:b.alignment?{...b.alignment,status:'done'}:null}:b);setBooks(bs=>bs.map(b=>b.id===book.id?{...b,status:'readalong'}:b));changeTextSource('book');setNotice('Aligned book text is ready. Your notes and captions were kept.');}}/></div>
 {book.readalong&&<div className="text-source"><span>Read from</span><Button variant={readingBook?'secondary':'ghost'} aria-pressed={readingBook} onClick={()=>changeTextSource('book')}>Book text</Button><Button variant={!readingBook?'secondary':'ghost'} aria-pressed={!readingBook} onClick={()=>changeTextSource('captions')}>Audio captions</Button><small>{readingBook?'Timed passages from your EPUB':'Generated or imported captions'}</small></div>}
 ...
 <div className="passage-pages"><Button variant="ghost" disabled={readPage===0} onClick={()=>{setReadPage(readPage-1);setFollow(false);}}>Earlier passages</Button><span>{filteredCues.length ? (readPage*80+1)+"–"+Math.min((readPage+1)*80,filteredCues.length)+" of "+filteredCues.length : "No matching passages"}</span><Button variant="ghost" disabled={(readPage+1)*80>=filteredCues.length} onClick={()=>{setReadPage(readPage+1);setFollow(false);}}>Later passages</Button></div><div className="transcript" onMouseUp={captureSelection}>{readingCues.length?visibleCues.map(({c,i})=>{const saved=book.notes.some(n=>n.quote.includes(c.text)&&c.start>=n.start-offset-.1&&c.end<=n.end-offset+.1);return <div key={i} id={'cue-'+i} className={'cue '+(active===i?'active ':'')+(selected.includes(i)?'selected ':'')+(saved?'saved':'')}><button className="cue-time" aria-label={'Play from '+time(c.start+offset)} onClick={()=>replay(c)}>{active===i?<Play/>:time(c.start+offset)}</button><p data-cue={i}>{c.text}</p><Checkbox aria-label={'Select sentence at '+time(c.start)} checked={selected.includes(i)} onCheckedChange={()=>selectCue(i)}/></div>;}):<div className="empty"><Headphones/><h2>Your book has a voice.<br/>Give it a margin.</h2><p>Pair an EPUB from your ebook folder, import a Storyteller readaloud, or generate captions to read along and save passages.</p><Button disabled={busy||Boolean(running)||status.demo} onClick={transcribe}>Generate captions</Button></div>}</div></TabsContent>
